...
apps/
    ...
    dashboard/        @monorepo/dashboard — local-only dashboard over this repo's docs and metrics
packages/
...
| --- | --- | --- |
| `type:app` | `type:feature`, `type:domain`, `type:ui`, `type:i18n`, `type:content`, `type:observability`, `type:config` | `huella-legal`, `dashboard` |
...
| `pnpm dashboard` | Start [`@monorepo/dashboard`](./apps/dashboard/README.md), the local dashboard over this repo's markdown, coverage, graph and issues |
| `pnpm dashboard:collect` | Rebuild the snapshot that dashboard reads |
