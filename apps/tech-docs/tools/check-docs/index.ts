...
import { spikeProblems } from "../lib/spikes.ts";
...
const SPIKES_DIR = "docs/spikes";

/** `README.md` and `TEMPLATE.md` are the furniture around the reports, and neither is one. */
async function readSpikeReports(): Promise<{ file: string, source: string }[]> {
    const dir = resolve(WORKSPACE_ROOT, SPIKES_DIR);
 ...
 * dashboard read, so a broken link, a drifted invariant or a malformed spike fails the build instead
 * of only showing up on a page nobody's looking at. Shells out to `nx graph`, never `gh` — no token
 * needed in the job env.
 */
async function main(): Promise<void> {
    ...
    const spikes = spikeProblems(reports);

    if (docs.brokenLinkCount === 0 && invariantFindings.length === 0 && spikes.length === 0) {
        console.log(`✓ check-docs — ${docs.pages.length} pages, ${reports.length} spikes, no findings`);
    ...
    if (spikes.length > 0) {
        console.error(`✗ ${spikes.length} spike report problem(s):`);

        for (const problem of spikes) {
            console.error(`  ${SPIKES_DIR}/${problem.file}: ${problem.message}`);
        }
    }
