...
const warnFor = async (root: string): Promise<void> => {
    ...
    log.warn(`${root} changed a lot and its docs might be stale.`);
    ...
    if (!process.stdout.isTTY) {
        log.info("Run `pnpm docs:drift` to review and file an issue.");
        return;
    }

    intro(`📚 Possible docs drift — ${name}`);

    const wantsIssue = orExit(await confirm({ message: "File a GitHub issue for it?" }), "Skipped.");

    if (!wantsIssue) {
        outro("Skipped — won't ask again until this project changes further.");
        return;
    }
